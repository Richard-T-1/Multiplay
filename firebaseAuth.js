...
function addCreditToUser(amount) {
    ...
    firebase.database().ref(window.authState.userName).once('value')
        .then((snapshot) => {
            ...
            const updatedData = {
                displayName: window.authState.userName,
                email: window.authState.userEmail,
                credit: window.credit,
                hry: userData.hry || {},
                lastUpdated: formatDate(new Date()),
                uid: window.authState.userId
            };
            ...
            return firebase.database().ref(window.authState.userName).update(updatedData);
        })
        .then(() => {
...
function saveUserCredit() {
    if (!window.authState || !window.authState.isLoggedIn || !window.authState.userId) {
